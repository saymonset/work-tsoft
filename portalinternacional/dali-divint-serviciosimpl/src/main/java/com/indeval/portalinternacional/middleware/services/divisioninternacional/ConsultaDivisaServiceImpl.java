...
public class ConsultaDivisaServiceImpl implements ConsultaDivisaService {
    ...
    @Override
    public HashMap<String, DivisaDTO> consultarDivisas() {
        HashMap<String, DivisaDTO> divisas = new HashMap<>();
        List<DivisaInt> resultadosConsulta = divisaDaliDAO.consultarDivisas();
        for (DivisaInt divisa : resultadosConsulta) {
            divisas.put(divisa.getClaveAlfabetica().toUpperCase(),
                    DTOAssembler.crearDivisaDTO(divisa));
        }
        return divisas;
    }
     ...
     *
     * @param divisaDaliDAO el campo divisaDaliDAO a establecer
     */
