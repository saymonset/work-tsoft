...
public class MenuNavegacionHelper {
	...
	private static final Logger log = LoggerFactory.getLogger(	MenuNavegacionHelper.class);
	...
	public static ElementoMenu obtenerMenuNavegacionDeUsuario(ElementoMenu elementoRaiz, String usuario, String ticket) {
		...
		try {
			...
			log.debug("Roles para el tiket ["+ticket+"]  ["+roles+"]");
			log.debug("Facultades para el tiket ["+ticket+"]  ["+facultades+"]");		
			...
			log.error("Ocurrió un error al iniciar sesión en el sistema.", e);
		}  catch (Exception e) {
			log.error("Error al invocar el EJB de seguridad.", e);
		}
