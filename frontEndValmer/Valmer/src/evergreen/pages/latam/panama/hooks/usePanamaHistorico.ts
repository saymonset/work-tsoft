...
import { RespHistoricoConsultaTabla, RespHistoricoDownloadCsv, ResponseCamposHistorico } from "../../../../../model";
...
export const usePanamaHistorico = () => {
  const { nemoTecnicoHistorico,
          loadingNemo,
          setNemoTecnicoHistorico,
          setLoadingNemo,
          setNemoTecnicoLoaded
        } = useNemotecnicos();
  const { campos, loadingCampos, setCampos, setCamposLoaded } = useCampos();
  ...
  const [allNemotecnicoOptions, setAllNemotecnicoOptions] = useState<string[]>([]);
