...
/**
 * Utileria para la consulta de posicion controlada.
 * 
 * @author Pablo Balderas
 */
public class ConsultaPosicionControladaUtil {

	/** Id de opcion todos en los combos */
	private static final String ID_TODOS = "-1"; 

	/**
	 * Metodo que crea el DetachedCriteria para la consulta de posicion controlada.
	 ...
	 */
	public static DetachedCriteria crearDetachedCriteriaPosicionControlada(PosicionDTO criteriosBusqueda, boolean esHistorica, CriterioOrdenamientoDTO orden) {
	...
	/**
	 * Metodo que crea el DetachedCriteria para obtener la proyeccion de la consulta de posicion controlada.
	 * @param criteriosBusqueda Criterios para realizar la consulta.
	 * @param esHistorica Indica si es consulta historica.
	 * @return DetachedCriteria
	 */
	public static DetachedCriteria crearDetachedCriteriaProyeccionPosicionControlada(PosicionDTO criteriosBusqueda, boolean esHistorica) {
		DetachedCriteria criteria = crearDetachedCriteria(esHistorica);
		agregarCondiciones(criteria, criteriosBusqueda, esHistorica);
	...
	/**
	 * Metodo que crea el DetachedCriteria para obtener los totales de la posicion controlada .
	 * @param criteriosBusqueda Criterios para realizar la consulta.
	 * @param esHistorica Indica si es consulta historica.
	 * @return DetachedCriteria
	 */
	public static DetachedCriteria crearDetachedCriteriaTotalesPosicionControlada(PosicionDTO criteriosBusqueda, boolean esHistorica) {
		DetachedCriteria criteria = crearDetachedCriteria(esHistorica);
		agregarCondiciones(criteria, criteriosBusqueda, esHistorica);
	...
	private static DetachedCriteria crearDetachedCriteria(boolean esHistorica) {
		...
		if(esHistorica) {
			criteria = DetachedCriteria.forClass(VPosicionControladaH.class);
		}
		else {
			criteria = DetachedCriteria.forClass(VPosicionControlada.class);
		}
		return criteria;
	}
	...
	private static void agregarCondiciones(DetachedCriteria criteria, PosicionDTO criteriosBusqueda, boolean esHistorica) {
		if(criteriosBusqueda != null) {
			...
			if(criteriosBusqueda.getCuenta() != null) {
				if(criteriosBusqueda.getCuenta().getInstitucion() != null) {
...
//					criteria.add(Restrictions.eq("cuenta", criteriosBusqueda.getCuenta().getCuenta()));
	...
	/**
	 * Agrega las proyecciones necesarias para obtener los totales de la consulta de posicion controlada.
	...
	private static void agregarOrdenamiento(DetachedCriteria criteria, CriterioOrdenamientoDTO orden, boolean esHistorica) {
		if(orden != null && orden.getColumna() != null) {
			if(OrdenamientoConsulta.SORT_CUENTA.getOrdenamiento().equals(orden.getColumna())) {
				...
				if(orden.isAscendente()) {
					criteria.addOrder(Order.asc("boveda"));
				}
				else {
					criteria.addOrder(Order.desc("boveda"));
				}
			}
