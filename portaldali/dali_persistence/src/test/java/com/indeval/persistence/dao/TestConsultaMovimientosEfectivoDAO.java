 ...
 * 
 */
public class TestConsultaMovimientosEfectivoDAO extends BaseDaoTestCase{
	
	private Logger logger = LoggerFactory.getLogger(TestConsultaMovimientosEfectivoDAO.class);
	...
	public void testConsultaMovimientosEfectivo() {
		...
		try {
			fechaInicial = sdf.parse("04-05-2009 00:00:00");
			fechaFinal = sdf.parse("04-05-2009 23:59:59");
		} catch (Exception e) {
		...
		criterio.getCuenta().setNumeroCuenta("120012000");
		criterio.getCuenta().getInstitucion().setId(185);
...
//		criterio.getCuenta().getInstitucion().setFolioInstitucion("061");
		...
		criterio.setImporte(new BigDecimal("0.0"));
		
		criterio.setTipoRetiro("-1");
		...
		for( RegistroContableSaldoNombradaDTO saldo : registros ) {
			//log.info("Importe: [" + saldo.getCantidad() + "]");
		}
