 ...
 * Miscelanea Fiscal de Capitales.
 * 
 * @author José Antonio Huizar Moreno
 * @version 1.0
 * 
 */
public class TraspasoMiscelaneaFiscalCapitalesController extends CapturaOperacionesController {
	...
	public void enviarOperacion(ActionEvent e) {
		...
		try {				
			logger.trace("Entra a enviarOperacion() boveda: " + traspasoMiscelaneaFiscal.getPosicionTraspasante().getBoveda());
	...
	private void enviarOperacionABitacora() {
		boolean isCompra = traspasoMiscelaneaFiscal.getRecepcion() != null ? traspasoMiscelaneaFiscal.getRecepcion() : false;
		...
		if( null != traspasoMiscelaneaFiscal.getPosicionTraspasante().getBoveda()  &&  traspasoMiscelaneaFiscal.getPosicionTraspasante().getBoveda().getId() >0  ){
			BovedaDTO boveda = consultaCatalogosFacade.buscarBovedaPorId(traspasoMiscelaneaFiscal.getPosicionTraspasante().getBoveda().getId());		
			tlpvo.setBoveda( null != boveda ?  boveda.getNombreCorto() : null);
		}else{
			tlpvo.setBoveda(null);
		}
	...
	private void calcularYRegistrarOperacicon() {
		...
		params.setFechaAdquisicion(new Date());//se elimina peticion de la pantalla, pero se necesita enviar al adaptador
		...
		params.setCostoPromedio(traspasoMiscelaneaFiscal.getCostoPromedio() != null ? new BigDecimal(traspasoMiscelaneaFiscal.getCostoPromedio().toString()) : null);
		
		// params.setAceptaCargo(traspasoMiscelaneaFiscal.getAceptaCargo().booleanValue());
		// params.setTipoMovimiento("TRASPASO");

		if (mercadoCapitalesService != null) {
			String tipoOperacionLocal = "M"; // Misc Fisc Mercado Dinero
			params.setTipoOperacion(tipoOperacionLocal);
			
			folioAsignado = mercadoCapitalesService.businessRulesCapturaTraspaso(params);
			...
			if (!existeErrorEnInvocacion()) {
				...
				tlpvo.setCostoFiscalActualizado(params.getCostoPromedio());
	 ...
	 * @return
	 */
	private TraspasoLibrePagoVO llenaVO(GetCapturaTraspasoParams params) {
	...
	public String obtenerDatosPosicion() {
		...
		if (StringUtils.isNotEmpty(traspasoMiscelaneaFiscal.getPosicionTraspasante().getCuenta().getCuenta())
			...
			if (institucion != null) {
				...
				if (traspasoMiscelaneaFiscal.getPosicionTraspasante().getBoveda() != null
						&& traspasoMiscelaneaFiscal.getPosicionTraspasante().getBoveda().getId() > 0) {
				...
				if (listaPosiciones != null && !listaPosiciones.isEmpty()) {
					...
					if(traspasoMiscelaneaFiscal.getPosicionTraspasante().getEmision() != null) {
						traspasoMiscelaneaFiscal.getPosicionTraspasante().getEmision().setIsin(null);
					}
