 ...
 * @version 1.0
 */
public class VentaCapitalesController extends CapturaOperacionesController {
	...
	/**
	 * Validador Mismo Dia, depende del valor del ratioButton con la etiqueta
	 * "Mismo día" (MD)
	 */
	private DTOValidator validadorMD;

	/**
	 * Validador Fecha Valor, depende del valor del ratioButton con la etiqueta
	 * "Fecha Valor" (FV)
	 */
	private DTOValidator validadorFV;

	/**
	 * Validador Mismo Dia Compra en Directo, depende del valor del ratioButton
	 * con la etiqueta "Mismo Día" (MD) y del checkBox con la etiqueda "Compra
	 * en Directo" (CD)
	 */
	private DTOValidator validadorMDCD;

	/**
	 * Validador Fecha Valor Compra en Directo, depende del valor del
	 * ratioButton con la etiqueta "Fecha Valor" (FV) y del checkBox con la
	 * etiqueda "Compra en Directo" (CD)
	 */
	private DTOValidator validadorFVCD;

	/**
	 * El DTO que representa a los elementos de la pantalla de venta, con el
	 * detalle de los datos del Traspasante  participante y del Receptor 
	 * contraparte, as como los campos necesarios con la información necesaria
	 * para realizar la captura de la operación.
	 */
	...
	/** Operación Mismo día (MD) o Fecha Valor (FV) */
	...
	public String getInitForm() {
		...
		venta.setIdFolioTraspasante(getInstitucionActual().getClaveTipoInstitucion() + getInstitucionActual().getFolioInstitucion());
	 ...
	 *         <code>false</code> si la compra se realiza segun la fecha.
	 */
	...
	/**
	 * Valida que la cuenta del Transpasante no pede ser igual a la cuenta del
	 * Receptor
	 *
	 * @param e
	 */
	public String validarCuenta(ActionEvent e) {
		...
		if (venta.getPosicionTraspasante().getCuenta().getCuenta() != null && !venta.getPosicionTraspasante().getCuenta().getCuenta().equals("")) {
			if (venta.getPosicionTraspasante().getCuenta().getCuenta().equals(venta.getCuentaReceptor().getCuenta())
					&& venta.getPosicionTraspasante().getCuenta().getInstitucion().getFolioInstitucion().equals(
							venta.getCuentaReceptor().getInstitucion().getFolioInstitucion())) {
				...
				if (isCompraEnDirecto() == Boolean.TRUE) {
					venta.getPosicionTraspasante().getCuenta().setCuenta("");
				} else {
	...
	public void cambioCompraDirecto(ActionEvent event) {
		...
		if (venta.getCompraDirecto() != null && venta.getCompraDirecto().booleanValue()) {
			...
			venta.setIdFolioTraspasante(StringUtils.EMPTY);
			venta.setCuentaReceptor(new CuentaDTO());
			venta.setIdFolioReceptor(getInstitucionActual().getClaveTipoInstitucion() + getInstitucionActual().getFolioInstitucion());
	...
	public void enviarOperacion(ActionEvent e) {
		...
		try {
			if (StringUtils.isEmpty(validarCuenta(e))) {
				venta.getPosicionTraspasante().getCuenta().setInstitucion(consultaCatalogosFacade.buscarInstitucionPorIdNoNulaFolio(venta.getIdFolioTraspasante()));
				venta.getCuentaReceptor().setInstitucion(consultaCatalogosFacade.buscarInstitucionPorIdNoNulaFolio(venta.getIdFolioReceptor()));
	...
	private void enviarOperacionABitacora() {
		...
		DivisaDTO divisaDTO = consultaCatalogosFacade.buscarDivisaPorId(venta.getValorEn().getId());
		vo.setDivisa( null != divisaDTO ? divisaDTO.getClaveAlfabetica() : null );
		BovedaDTO bovedaDTO = null;
		if(venta.getPosicionTraspasante().getBoveda() != null && venta.getPosicionTraspasante().getBoveda().getId() > 0) {
			bovedaDTO = consultaCatalogosFacade.buscarBovedaPorId(venta.getPosicionTraspasante().getBoveda().getId());
		}
		vo.setBoveda( null != bovedaDTO ? bovedaDTO.getNombreCorto() : null);

		if(venta.getBovedaEfectivo() != null && venta.getBovedaEfectivo().getId() >0){
			bovedaDTO = consultaCatalogosFacade.buscarBovedaPorId(venta.getBovedaEfectivo().getId());
		}else{
			bovedaDTO = null;
		}

		vo.setBovedaEfectivo(null != bovedaDTO ? bovedaDTO.getNombreCorto() : null);

		envioOperacionesService.grabaOperacion(vo, folioAsignado, venta.getCompraDirecto() != null ? venta.getCompraDirecto().booleanValue() : false, datosAdicionales, null, isoFirmado);
	...
	public void realizarCalculo(ActionEvent e) {
		...
		calculos.setCantidadOperada(new BigDecimal(venta.getCantidadOperada() != null ? venta.getCantidadOperada().doubleValue() : 0));
		...
		calculos.setPlazoLiquidacionHorasLeido(venta.getPlazoLiquidacionHoras() != null ? venta.getPlazoLiquidacionHoras().toString() : "");
		...
		/*
		 * Valida que el importe no sea mayor al saldo disponible (por ahora no
		 * se valida)
		 */
		/*
		 * if(venta.getSaldoDisponible() != null) { if (
		 * venta.getSaldoDisponible().longValue() < venta.getImporte()) {
	...
	private void calcularYRegistrarOperacicon() {
		...
		GetCapturaTraspasoContraPagoParams registraOperacionParams = new GetCapturaTraspasoContraPagoParams();

		if(null != venta.getPosicionTraspasante().getBoveda() &&  venta.getPosicionTraspasante().getBoveda().getId() > 0  ){
		...
		registraOperacionParams.setCveReporto("V");
		...
		registraOperacionParams.setCantidad(new BigDecimal(venta.getCantidadOperada()));
		registraOperacionParams.setFechaLiquidacion(venta.getFechaConcertacion());
		registraOperacionParams.setUsuario(registraOperacionParams.getTraspasante().getId());
		registraOperacionParams.setNombreUsuario(registraOperacionParams.getTraspasante().getNombreCorto());
		...
		if(venta.getPosicionTraspasante().getBoveda() != null){
			BovedaDTO boveda =  consultaCatalogosFacade.buscarBovedaPorId(venta.getPosicionTraspasante().getBoveda().getId());
			venta.getPosicionTraspasante().setBoveda(boveda);
		}
		...
		// --- TIPO VALOR
		registraOperacionParams.getEmision().setIdTipoValor(venta.getPosicionTraspasante().getEmision().getTipoValor().getClaveTipoValor());
		// --- EMISORA
		registraOperacionParams.getEmision().setEmisora(venta.getPosicionTraspasante().getEmision().getEmisora().getDescripcion());
		// --- SERIE
		...
		if (venta.getPlazoLiquidacionHoras() != null) {
			registraOperacionParams.setLiquidacion(venta.getPlazoLiquidacionHoras());
		} else {
			...
			registraOperacionParams.setLiquidacion(Integer.valueOf("0"));
		}
		...
		registraOperacionParams.setFechaLiquidacion((servicesCapturaOperacionViewHelper.calculaFechaLiquidacion(registraOperacionParams.getLiquidacion())));
		...
		if (!encontroError) {
			...
			if (this.configuracion.getTiposOperacionesFirmadas().contains(registraOperacionParams.getCveReporto())) {

				folioAsignado = mercadoCapitalesService.businessRulesCapturaTraspasoContraPago(registraOperacionParams);
				if (!existeErrorEnInvocacion()) {
					...
					vo.setBoveda((venta.getPosicionTraspasante().getBoveda() == null) ? null : venta.getPosicionTraspasante().getBoveda().getNombreCorto());
                    vo.setBovedaEfectivo(venta.getBovedaEfectivo() != null ? venta.getBovedaEfectivo().getNombreCorto() : null);
	...
	/**
	 * Crea un objeto TraspasoContraPagoVO y lo llena con los valores del objeto
	 * registraOperacionParams. Este m&eacute;todo tiene como objetivo eliminar
	 * la redundancia de codigo que puede existir al momento de incializar el
	 * objeto vo en cada una de los tipos de operaciones
	 *
	 * @param registraOperacionParams
	 *            Objeto de donde se tomara la informacion
	 * @return vo con los campos inicializados en base al objeto proporcionado
	 *         como parametros
	 */
	private TraspasoContraPagoVO inicializaTraspasoContrapagoVO(GetCapturaTraspasoContraPagoParams registraOperacionParams) {
		...
		vo.setIdFolioCtaReceptora(registraOperacionParams.getReceptor().getId() + registraOperacionParams.getReceptor().getFolio()
				+ registraOperacionParams.getReceptor().getCuenta());
		vo.setIdFolioCtaTraspasante(registraOperacionParams.getTraspasante().getId() + registraOperacionParams.getTraspasante().getFolio()
				+ registraOperacionParams.getTraspasante().getCuenta());
		vo.setFechaLiquidacion(registraOperacionParams.getFechaLiquidacion());
		vo.setFechaConcertacion(registraOperacionParams.getFechaLiquidacion());// <-Extra
		...
		vo.setMonto(registraOperacionParams.getPrecioTitulo().multiply(registraOperacionParams.getCantidad()));
	...
	public String obtenerDatosPosicion() {
		...
		if (StringUtils.isNotEmpty(venta.getPosicionTraspasante().getCuenta().getCuenta())
			...
			if (institucion != null) {
				...
				if (StringUtils.isNotEmpty(venta.getPosicionTraspasante().getCuenta().getCuenta())) {
					criterio.getCuenta().setNumeroCuenta(
							institucion.getClaveTipoInstitucion() + institucion.getFolioInstitucion() + venta.getPosicionTraspasante().getCuenta().getCuenta());
				} else {
				...
				criterio.getEmision().setTipoValor(
						consultaCatalogosFacade.buscarTipoValorPorClave(venta.getPosicionTraspasante().getEmision().getTipoValor().getClaveTipoValor()));
				// Emisora
				criterio.getEmision().setEmisora(
						consultaCatalogosFacade.buscarEmisoraPorNombreCorto(venta.getPosicionTraspasante().getEmision().getEmisora().getDescripcion()));
				...
				List<PosicionDTO> listaPosiciones = consultaPosicionService.consultarPosicionesPorMercado(criterio, null, orden, idMercadoCapitales
						.toArray(new Long[] {}));
	...
	public TipoInstruccionDTO getTipoInstruccion(){
		return utilService.obtenerTipoInstruccionPorClaveOperacion("DVPC");
	}
	 ...
	 *
	 * @param mismoDiaFechaValor
	 *            el valor de mismoDiaFechaValor a asignar
	 */
	 ...
	 *
	 * @param validadorMD
	 *            el valor de validadorMD a asignar
	 */
	 ...
	 *
	 * @param validadorFV
	 *            el valor de validadorFV a asignar
	 */
	 ...
	 *
	 * @param validadorMDCD
	 *            el valor de validadorMDCD a asignar
	 */
	 ...
	 *
	 * @param validadorFVCD
	 *            el valor de validadorFVCD a asignar
	 */
	 ...
	 *
	 * @param idTraspasanteReadonly
	 *            el valor de idTraspasanteReadonly a asignar
	 */
	 ...
	 *
	 * @param folioTraspasanteReadonly
	 *            el valor de folioTraspasanteReadonly a asignar
	 */
	 ...
	 *
	 * @param idReceptorReadonly
	 *            el valor de idReceptorReadonly a asignar
	 */
	 ...
	 *
	 * @param folioReceptorReadonly
	 *            el valor de folioReceptorReadonly a asignar
	 */
	 ...
	 *
	 * @param plazoLiquidacionHorasInhabilitado
	 *            el valor de plazoLiquidacionHorasInhabilitado a asignar
	 */
	 ...
	 *
	 * @param isinInhabilitado
	 *            el valor de isinInhabilitado a asignar
	 */
	 ...
	 *
	 * @param precioVectorInhabilitado
	 *            el valor de precioVectorInhabilitado a asignar
	 */
	 ...
	 *
	 * @param simuladoInhabilitado
	 *            el valor de simuladoInhabilitado a asignar
	 */
	 ...
	 *
	 * @param saldoDisponibleInhabilitado
	 *            el valor de saldoDisponibleInhabilitado a asignar
	 */
	 ...
	 *
	 * @param netoEfectivoInhabilitado
	 *            el valor de netoEfectivoInhabilitado a asignar
	 */
	 ...
	 *
	 * @param diasVigentesInhabilitado
	 *            el valor de diasVigentesInhabilitado a asignar
	 */
	 ...
	 *
	 * @param plazoRepDiasInhabilitado
	 *            el valor de plazoRepDiasInhabilitado a asignar
	 */
	...
	public void setMercadoCapitalesService(MercadoCapitalesService mercadoCapitalesService) {
	 ...
	 *
	 * @param idMercadoCapitales
	 *            el valor del atributo idMercadoCapitales a establecer
	 */
