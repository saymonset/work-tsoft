...
/**
 * Controller para los Traspasos Administrativos de Mercado de Capitales
 * 
 * @author Erik Vera Montoya
 * 
 * @version 1.0
 */
public class TraspasosAdministrativosCapitalesController extends CapturaOperacionesController {
	/**
	...
	public String validarCuenta(ActionEvent e) {
		...
		if (traspasos.getPosicionTraspasante().getCuenta().getCuenta() != null && !traspasos.getPosicionTraspasante().getCuenta().getCuenta().equals("")) {
			if (traspasos.getPosicionTraspasante().getCuenta().getCuenta().equals(traspasos.getCuentaReceptor().getCuenta())
					&& traspasos.getPosicionTraspasante().getCuenta().getInstitucion().getFolioInstitucion().equals(
							traspasos.getCuentaReceptor().getInstitucion().getFolioInstitucion())) {
	...
	public void enviarOperacion(ActionEvent e) {
		...
		try {
			if (StringUtils.isEmpty(validarCuenta(e))) {
				traspasos.getPosicionTraspasante().getCuenta().setInstitucion(
						consultaCatalogosFacade.buscarInstitucionPorIdNoNulaFolio(traspasos.getIdFolioTraspasante()));
				traspasos.getCuentaReceptor().setInstitucion(consultaCatalogosFacade.buscarInstitucionPorIdNoNulaFolio(traspasos.getIdFolioReceptor()));
	...
	private void enviarOperacionABitacora() {
		...
		if( null != traspasos.getPosicionTraspasante().getBoveda() && traspasos.getPosicionTraspasante().getBoveda().getId()>0 ){
		tlpvo.setBoveda(consultaCatalogosFacade.buscarBovedaPorId(traspasos.getPosicionTraspasante().getBoveda().getId()).getNombreCorto());
		}
		...
		if (!existeErrorEnInvocacion()) {
			mensajeUsuario = "La informacion ha sido guardada y se ha generado el folio: " + folioAsignado;
			FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, mensajeUsuario, mensajeUsuario));
	...
	private TraspasoLibrePagoVO llenaVOTraspasoMercadoDinero(TraspasosAdministrativosParams params) {
		...
		if( null != traspasos.getPosicionTraspasante().getBoveda() && traspasos.getPosicionTraspasante().getBoveda().getId()>0 ){
			BovedaDTO boveda = consultaCatalogosFacade.buscarBovedaPorId(traspasos.getPosicionTraspasante().getBoveda().getId());
			vo.setBoveda(boveda != null ? boveda.getNombreCorto(): null);
		}
